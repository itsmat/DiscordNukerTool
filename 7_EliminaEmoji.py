...
impostatitolo(f"Mat Nuker Tool {VERSIONETOOL} - Elimina Emoji")
clear()
...
async def on_ready():
    ...
    for emojidiocane in serverr.emojis:
        try:
            await emojidiocane.delete()
            print(f'{y}[{b}MatNukerTool{y}]{g} Ho eliminato {emojidiocane} {w}')
        except Forbidden:
            print(f'{y}[{b}MatNukerTool{y}]{r} Non riesco ad eliminare {emojidiocane} [Mancanza di permessi]{w}')
        except:
            print(f'{y}[{b}MatNukerTool{y}]{r} Non riesco ad eliminare {emojidiocane}{w}')
