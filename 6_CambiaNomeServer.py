...
impostatitolo(f"Mat Nuker Tool {VERSIONETOOL} - Cambia nome server")
clear()
...
async def on_ready():
    ...
    try:
        await serverr.edit(name="NUKED | Made via MatNukerTool [github.com/itsmat]", community=False)
        print(f'{y}[{b}MatNukerTool{y}]{g} Ho cambiato il nome del server{w}')
    except Forbidden:
        print(f'{y}[{b}MatNukerTool{y}]{r} Non riesco a modificare il nome del server [Mancanza di permessi]{w}')
    except:
        print(f'{y}[{b}MatNukerTool{y}]{r} Non riesco a modificare il nome del server{w}')
