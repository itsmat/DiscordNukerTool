...
impostatitolo(f"Mat Nuker Tool {VERSIONETOOL} - Ban All")
clear()
...
async def on_ready():
    ...
    for membro in serverr.members:
        try:
            ...
            print(f'{y}[{b}MatNukerTool{y}]{g} Ho bannato {membro}{w}')
        except Forbidden:
            print(f'{y}[{b}MatNukerTool{y}]{r} Non riesco a bannare {membro} [Mancanza di permessi]{w}')
        except:
            print(f'{y}[{b}MatNukerTool{y}]{r} Non riesco a bannare {membro}{w}')
