...
impostatitolo(f"Mat Nuker Tool {VERSIONETOOL} - Elimina Ruoli")
clear()
...
async def on_ready():
    ...
    for ruolo in serverr.roles:
        try:
            await ruolo.delete()
            print(f'{y}[{b}MatNukerTool{y}]{g} Ho eliminato {ruolo}{w}')
        except Forbidden:
            print(f'{y}[{b}MatNukerTool{y}]{r} Non riesco ad eliminare {ruolo} [Mancanza di permessi]{w}')
        except:
            print(f'{y}[{b}MatNukerTool{y}]{r} Non riesco ad eliminare {ruolo}{w}')
